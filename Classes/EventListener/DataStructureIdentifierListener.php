...
/**
 * Event listener extending the flex form handling for tt_content form elements (CType: feregistration_registration):
 *
...
readonly class DataStructureIdentifierListener
{
     ...
     */
    #[AsEventListener('fe-registration/modify-data-structure-identifier')]
    public function modifyDataStructureIdentifier(AfterFlexFormDataStructureIdentifierInitializedEvent $event): void
    {
        $row = $event->getRow();
        if (($row['CType'] ?? '') !== 'feregistration_registration' || $event->getTableName() !== 'tt_content' || $event->getFieldName() !== 'pi_flexform') {
            return;
        }
        ...
        if (isset($currentFlexData['data']['sDEF']['lDEF']['settings.overrideFinishers']['vDEF'])
            ...
            $identifier['ext-feregistration-overrideFinishers'] = 'enabled';
        }
     ...
     */
    #[AsEventListener('fe-registration/modify-data-structure')]
    public function modifyDataStructure(AfterFlexFormDataStructureParsedEvent $event): void
    {

        $identifier = $event->getIdentifier();
        if (!isset($identifier['ext-feregistration-persistenceIdentifier'])) {
            return;
        }
